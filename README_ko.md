...
| Tomcat: 전체 배포 | 전체 동기화 재실행 (`Ctrl+Alt+D`) |
...
- 최초 사용 전 `mvn compile` 한 번 실행 필요 (확장 프로그램이 `target/classes`에서 동기화)
...
- 최초 사용 전 `gradle compileJava` 한 번 실행 필요 (확장 프로그램이 `build/classes`에서 동기화)
